...
function ProductFamilyUpdateList() {
  ...
  useEffect(() => {
    const editItem = JSON.parse(localStorage.getItem("editProductFamilyItem"));
    if (editItem) {
  ...
  const handleSubmit = (e) => {
    ...
    if (validateForm()) {
      ...
      localStorage.removeItem("editProductFamilyItem");

      alert(editId ? "Entry updated successfully!" : "Form submitted successfully!");
      navigate("/generalsetting/product/productfamilylist");
    }
  };
  ...
  return (
    <div className="form-container">
      <div className='list-bt-div'>
        <h2>{editId ? 'Edit ProductFamily' : 'ProductFamily Update List'}</h2>
        <div>
...
export default ProductFamilyUpdateList;
