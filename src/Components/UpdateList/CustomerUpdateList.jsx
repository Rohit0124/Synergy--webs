...
function CustomerUpdateList() {
  ...
  useEffect(() => {
    const editItem = JSON.parse(localStorage.getItem("editCustomerItem"));
    if (editItem) {
  ...
  const handleSubmit = (e) => {
    ...
    if (validateForm()) {
      ...
      localStorage.setItem("cutomerData", JSON.stringify(updatedData));
      localStorage.removeItem("editCustomerItem");
  ...
  return (
    <div className="form-container">
      <div className='list-bt-div'>
        <h2>{editId ? 'Edit Customer' : 'Customer Update List'}</h2>
        <div>
...
export default CustomerUpdateList;
