...
function StateUpdateList() {
  ...
  useEffect(() => {
    const editItem = JSON.parse(localStorage.getItem("editStateItem"));
    if (editItem) {
  ...
  const handleSubmit = (e) => {
    ...
    if (validateForm()) {
      ...
      const storedData = JSON.parse(localStorage.getItem("StateData")) || [];
      ...
      localStorage.setItem("StateData", JSON.stringify(updatedData));
      localStorage.removeItem("editStateItem");

      alert(editId ? "Entry updated successfully!" : "Form submitted successfully!");
      navigate("/generalsetting/Statelist");
    }
  };
  ...
  return (
    <div className="form-container">
      <div className='list-bt-div'>
        <h2>{editId ? 'Edit State' : 'State Update List'}</h2>
        <div>
...
export default StateUpdateList;
