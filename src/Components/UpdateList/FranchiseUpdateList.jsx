...
function FranchiseUpdateList() {
  ...
  useEffect(() => {
    const editItem = JSON.parse(localStorage.getItem("editFranchiseItem"));
    if (editItem) {
  ...
  const handleSubmit = (e) => {
    ...
    if (validateForm()) {
      ...
      localStorage.removeItem("editFranchiseItem");

      alert(editId ? "Entry updated successfully!" : "Form submitted successfully!");
      navigate("/master/franchises/franchiseslist");
    }
  };
  ...
  return (
    <div className="form-container">
      <div className='list-bt-div'>
        <h2>{editId ? 'Edit Franchise' : 'Franchise Update List'}</h2>
        <div>
...
export default FranchiseUpdateList;
