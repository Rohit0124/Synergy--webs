...
function UserGroupUpdateList() {
  ...
  useEffect(() => {
    const editItem = JSON.parse(localStorage.getItem("editUserGroupItem"));
    if (editItem) {
  ...
  const handleSubmit = (e) => {
    ...
    if (validateForm()) {
      ...
      localStorage.removeItem("editUserGroupItem");

      alert(editId ? "Entry updated successfully!" : "Form submitted successfully!");
      navigate("/master/usergroups/usergrouplist");
    }
  };
  ...
  return (
    <div className="form-container">
      <div className='list-bt-div'>
        <h2>{editId ? 'EditUserGroup' : 'UserGroup Update List'}</h2>
        <div>
...
export default UserGroupUpdateList;
