...
function ProductUpdateList() {
  ...
  useEffect(() => {
    const editItem = JSON.parse(localStorage.getItem("editProductItem"));
    if (editItem) {
  ...
  const handleSubmit = (e) => {
    ...
    if (validateForm()) {
      ...
      localStorage.removeItem("editProductItem");

      alert(editId ? "Entry updated successfully!" : "Form submitted successfully!");
      navigate("/generalsetting/product/productlist");
    }
  };
  ...
  return (
    <div className="form-container">
      <div className='list-bt-div'>
        <h2>{editId ? 'Edit Product' : 'Product Update List'}</h2>
        <div>
