...
function OilCompanyUpdateList() {
  ...
  useEffect(() => {
    const editItem = JSON.parse(localStorage.getItem("editOilCompanyItem"));
    if (editItem) {
  ...
  const handleSubmit = (e) => {
    ...
    if (validateForm()) {
      ...
      localStorage.removeItem("editOilCompanyItem");

      alert(editId ? "Entry updated successfully!" : "Form submitted successfully!");
      navigate("/generalsetting/oilcompany/oilcompanylist");
    }
  };
  ...
  return (
    <div className="form-container">
      <div className='list-bt-div'>
        <h2>{editId ? 'Edit OilCompany' : 'OilCompany Update List'}</h2>
        <div>
...
export default OilCompanyUpdateList;
