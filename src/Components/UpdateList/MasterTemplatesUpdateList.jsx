...
function MasterTemplateUpdateList() {
  ...
  useEffect(() => {
    const editItem = JSON.parse(localStorage.getItem("editMasterTemplateItem"));
    if (editItem) {
  ...
  const handleSubmit = (e) => {
    ...
    if (validateForm()) {
      ...
      localStorage.removeItem("editMasterTemplateItem");

      alert(editId ? "Entry updated successfully!" : "Form submitted successfully!");
      navigate("/master/mastertemplate/mastertemplatelist");
    }
  };
  ...
  return (
    <div className="form-container">
      <div className='list-bt-div'>
        <h2>{editId ? 'EditMasterTemplate' : 'MasterTemplate Update List'}</h2>
        <div>
...
export default MasterTemplateUpdateList;
