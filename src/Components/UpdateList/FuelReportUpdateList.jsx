...
function FuelReportUpdateList() {
  ...
  useEffect(() => {
    const editItem = JSON.parse(localStorage.getItem("editFuelReportItem"));
    if (editItem) {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    if (validateForm()) {
      const storedData = JSON.parse(localStorage.getItem("fuelReportData")) || [];
      ...
      localStorage.setItem("fuelReportData", JSON.stringify(updatedData));
      localStorage.removeItem("editFuelReportItem");

      alert(editId ? "Entry updated successfully!" : "Form submitted successfully!");
      navigate("/MIS/receivefuelreports");
    }
  };
  ...
  return (
    <div className="form-container">
      <div className='list-bt-div'>
        <h2>{editId ? 'Edit FuelReport' : 'FuelReport Update List'}</h2>
        <div>
...
export default FuelReportUpdateList;
