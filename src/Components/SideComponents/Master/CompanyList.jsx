...
import { CompanyListData } from '../../../DataTable';
...
function CompanyList() {
  ...
  useEffect(() => {
    ...
    if (storedData && storedData.length > 0) {
      ...
      localStorage.setItem("companyData", JSON.stringify(CompanyListData));
    }
  }, []);

  const handleClick = () => {
    localStorage.removeItem("editCompany"); 
    navigate("/CompanyUpdateList");
  };
    ...
    <div>
      <div className='list-container'>
        <div className='list-bt-div'>
          <h2>Company List</h2>
          <div>
            <button className='list-add-bt' onClick={handleClick}>Add Baud Rate</button>
                ...
                <tr key={item.id}>
                  {editId === item.id ? (
                    <>
                      ...
                      <td><input name="personName" value={editItem.personName} onChange={handleEditChange} /></td>
