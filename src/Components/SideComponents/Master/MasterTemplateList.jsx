...
function MasterTemplateList() {
  ...
  useEffect(() => {
    const storedData = JSON.parse(localStorage.getItem("masterTemplateData"));
    if (storedData && storedData.length > 0) {
      setData(storedData);
    } else {
      setData(MasterTemplateData);
      localStorage.setItem("masterTemplateData", JSON.stringify(MasterTemplateData));
    }
  }, []);

  const handleClick = () => {
    localStorage.removeItem("editMasterTemplate"); 
    navigate("/MasterTemplateUpdateList");
  };
  ...
  const handleDelete = (id) => {
    if (window.confirm('Are you sure you want to delete this item?')) {
      ...
      setData(updatedData);
      localStorage.setItem("masterTemplateData", JSON.stringify(updatedData));
    }
  };
  ...
  const handleEditClick = (item) => {
    ...
    localStorage.setItem("editMasterTemplate", JSON.stringify(item));
    navigate("/MasterTemplateUpdateList");
  };
    ...
    <div>
      <div className='list-container'>
        <div className='list-bt-div'>
          <h2>Master Template List</h2>
          <div>
            <button className='list-add-bt' onClick={handleClick}>Add MasterTemplate</button>
                ...
                <tr key={item.id}>
                  {editId === item.id ? (
                    <>
                      ...
                      <td><input name="date" value={editItem.data} onChange={handleEditChange} /></td>
                      <td>
