...
function ProductFamilyList() {
  ...
  useEffect(() => {
    const storedData = JSON.parse(localStorage.getItem("ProductFamilyData"));
    if (storedData && storedData.length > 0) {
      setData(storedData);
    } else {
      setData(CountryListData);
      localStorage.setItem("ProductFamilyData", JSON.stringify(CountryListData));
    }
  }, []);

  const handleClick = () => {
    localStorage.removeItem("editProductFamily"); 
    navigate("/ProductFamilyUpdateList");
  };
  ...
  const handleDelete = (id) => {
    if (window.confirm('Are you sure you want to delete this item?')) {
      ...
      setData(updatedData);
      localStorage.setItem("ProductFamilyData", JSON.stringify(updatedData));
    }
  };
  ...
  const handleEditClick = (item) => {
    localStorage.setItem("editProductFamily", JSON.stringify(item));
    navigate("/ProductFamilyUpdateList");
  };
    ...
    <div>
      <div className='list-container'>
        <div className='list-bt-div'>
          ...
          <div>
            <button className='list-add-bt' onClick={handleClick}>Add Product</button>
