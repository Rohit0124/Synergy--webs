...
function DeviceMasterList() {
  ...
  useEffect(() => {
    const storedData = JSON.parse(localStorage.getItem("DeviceMasterData"));
    if (storedData && storedData.length > 0) {
      setData(storedData);
    } else {
      setData(CountryListData);
      localStorage.setItem("DeviceMasterData", JSON.stringify(CountryListData));
    }
  }, []);

  const handleClick = () => {
    localStorage.removeItem("editDeviceMaster"); 
    navigate("/DeviceMasterUpdateList");
  };
  ...
  const handleDelete = (id) => {
    if (window.confirm('Are you sure you want to delete this item?')) {
      ...
      setData(updatedData);
      localStorage.setItem("DeviceMasterData", JSON.stringify(updatedData));
    }
  };
  ...
  const handleEditClick = (item) => {
    localStorage.setItem("editDeviceMaster", JSON.stringify(item));
    navigate("/DeviceMasterUpdateList");
  };
    ...
    <div>
      <div className='list-container'>
        <div className='list-bt-div'>
          <h2>Device Master List</h2>
          <div>
            <button className='list-add-bt' onClick={handleClick}>Add DeviceMaster</button>
