...
function DeviceMakeList() {
  ...
  useEffect(() => {
    const storedData = JSON.parse(localStorage.getItem("DeviceMakeData"));
    if (storedData && storedData.length > 0) {
      setData(storedData);
    } else {
      setData(CountryListData);
      localStorage.setItem("DeviceMakeData", JSON.stringify(CountryListData));
    }
  }, []);

  const handleClick = () => {
    localStorage.removeItem("editDeviceMake"); 
    navigate("/DeviceMakeUpdateList");
  };
  ...
  const handleDelete = (id) => {
    if (window.confirm('Are you sure you want to delete this item?')) {
      ...
      setData(updatedData);
      localStorage.setItem("DeviceMakeData", JSON.stringify(updatedData));
    }
  };
  ...
  const handleEditClick = (item) => {
    localStorage.setItem("editDeviceMake", JSON.stringify(item));
    navigate("/DeviceMakeUpdateList");
  };
    ...
    <div>
      <div className='list-container'>
        <div className='list-bt-div'>
          <h2>Device Make List</h2>
          <div>
            <button className='list-add-bt' onClick={handleClick}>Add DeviceMake</button>
