...
function OilCompanyList() {
  ...
  useEffect(() => {
    const storedData = JSON.parse(localStorage.getItem("OilCompanyData"));
    if (storedData && storedData.length > 0) {
      setData(storedData);
    } else {
      setData(CountryListData);
      localStorage.setItem("OilCompanyData", JSON.stringify(CountryListData));
    }
  }, []);

  const handleClick = () => {
    localStorage.removeItem("editOilCompany"); 
    navigate("/OilCompanyUpdateList");
  };
  ...
  const handleDelete = (id) => {
    if (window.confirm('Are you sure you want to delete this item?')) {
      ...
      setData(updatedData);
      localStorage.setItem("OilCompanyData", JSON.stringify(updatedData));
    }
  };
  ...
  const handleEditClick = (item) => {
    localStorage.setItem("editOilCompany", JSON.stringify(item));
    navigate("/OilCompanyUpdateList");
  };
    ...
    <div>
      <div className='list-container'>
        <div className='list-bt-div'>
          <h2>Oil Company List</h2>
          <div>
            <button className='list-add-bt' onClick={handleClick}>Add OilCompany</button>
