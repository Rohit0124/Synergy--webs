...
function CountryList() {
  ...
  useEffect(() => {
    ...
    if (storedData && storedData.length > 0) {
      ...
      localStorage.setItem("CountryData", JSON.stringify(CountryListData));
    }
  }, []);

  const handleClick = () => {
    localStorage.removeItem("editCountry"); 
    navigate("/CountryUpdateList");
  };
  ...
  const handleEditClick = (item) => {
    localStorage.setItem("editCountry", JSON.stringify(item));
    navigate("/CountryUpdateList");
  };
    ...
    <div>
      <div className='list-container'>
        <div className='list-bt-div'>
          <h2>Country List</h2>
          <div>
            <button className='list-add-bt' onClick={handleClick}>Add Country</button>
                ...
                <tr key={item.id}>
                  {editId === item.id ? (
                    <>
                      ...
                      <td><input name="ISO2" value={editItem.ISO2} onChange={handleEditChange} /></td>
                      <td><input name="ISO3" value={editItem.ISO3} onChange={handleEditChange} /></td>
                      <td><input name="phoneCode" value={editItem.phoneCode} onChange={handleEditChange} /></td>
                      <td><input name="capital" value={editItem.capital} onChange={handleEditChange} /></td>
                      <td><input name="region" value={editItem.region} onChange={handleEditChange} /></td>
                      <td>
