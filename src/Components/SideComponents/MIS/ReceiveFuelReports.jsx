...
function ReceiveFuelReports() {
  ...
  useEffect(() => {
    const storedData = JSON.parse(localStorage.getItem("ReceiveFuelData"));
    if (storedData && storedData.length > 0) {
      setData(storedData);
    } else {
      setData(ReceiveFuelReportData);
      localStorage.setItem("ReceiveFuelData", JSON.stringify(ReceiveFuelReportData));
    }
  }, []);
  ...
  const handleSubmit = (e) => {
    ...
    localStorage.setItem("ReceiveFuelData", JSON.stringify(updatedData));
    setData(updatedData);
...
export default ReceiveFuelReports;
