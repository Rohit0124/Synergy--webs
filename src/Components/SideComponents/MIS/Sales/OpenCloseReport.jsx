...
function OpenCloseReport() {
  ...
  useEffect(() => {
    const storedData = JSON.parse(localStorage.getItem("OpenCloseData"));
    if (storedData && storedData.length > 0) {
      setData(storedData);
    } else {
      setData(ReceiveFuelReportData);
      localStorage.setItem("OpenCloseData", JSON.stringify(ReceiveFuelReportData));
    }
  }, []);
  ...
  const handleSubmit = (e) => {
    ...
    localStorage.setItem("OpenCloseData", JSON.stringify(updatedData));
    setData(updatedData);
...
export default OpenCloseReport;
