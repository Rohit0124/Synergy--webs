...
function PaymentSummaryReport() {
  ...
  useEffect(() => {
    const storedData = JSON.parse(localStorage.getItem("PaymentSummaryData"));
    if (storedData && storedData.length > 0) {
      setData(storedData);
    } else {
      setData(ReceiveFuelReportData);
      localStorage.setItem("PaymentSummaryData", JSON.stringify(ReceiveFuelReportData));
    }
  }, []);
  ...
  const handleSubmit = (e) => {
    ...
    localStorage.setItem("PaymentSummaryData", JSON.stringify(updatedData));
    setData(updatedData);
