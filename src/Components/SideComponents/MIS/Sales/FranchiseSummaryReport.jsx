...
function FranchiseSummaryReport() {
  ...
  useEffect(() => {
    const storedData = JSON.parse(localStorage.getItem("FranchiseSummaryData"));
    if (storedData && storedData.length > 0) {
      setData(storedData);
    } else {
      setData(ReceiveFuelReportData);
      localStorage.setItem("FranchiseSummaryData", JSON.stringify(ReceiveFuelReportData));
    }
  }, []);
  ...
  const handleSubmit = (e) => {
    ...
    localStorage.setItem("FranchiseSummaryData", JSON.stringify(updatedData));
    setData(updatedData);
...
export default FranchiseSummaryReport;
